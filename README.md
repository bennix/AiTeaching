...
- 自定义 OpenAI 兼容 `BaseURL`，获取并筛选适合教学工作的模型，可分别指定学生答案判题模型与题目复核模型
...
- 数学、物理、化学题目强制由主模型生成两种独立解法，再交给不同的复核模型验算；只有题干、答案和双解均通过复核后才会录入题库
- 完整保留教师端：教案、题库、课件、学生、班级资料、签到、报告、邮件和 AI 设置
- 教师端可按课程、班级和教学周分析签到率、作答完成率、正确率及知识点掌握度，并生成可保存的 AI 班级学情报告
- 可直接导入学校选课点名表，一键建立教学班；教案、题库和 AI 课件可同时关联多个班级
...
默认 ZenMux BaseURL 为 `https://zenmux.ai/api/v1`。应用会根据模型接口返回的模态与上下文信息，只列出支持文本输入、文本输出且上下文不少于 64K 的模型，并排除图像、视频、Embedding、Rerank、语音和转写模型。其他只返回模型 ID 的 OpenAI 兼容服务会自动采用兼容模式，模型 ID 仍可手动填写。

未配置 ZenMux API Key 时，设置页面会显示邀请入口：<https://zenmux.ai/invite/GBQMC5>。切换 BaseURL 时不会把原服务商的 API Key 发送给新的服务地址。
