...
const { lessonClassNames } = require('./teaching-catalog');

function catalogNames(values) {
  return [...new Map(values.map(text).filter(Boolean).map((name) => [normalizeCatalogName(name), name])).values()];
}
...
function buildLearningAnalytics(state = {}, requestedFilters = {}) {
  ...
  const completedLessons = allLessons.filter((lesson) => lesson.status === 'done' || text(lesson.aiResult || lesson.structuredNotes));
  const courses = catalogNames([
  ...
  const courseName = courses.find((name) => sameCatalogName(name, requestedCourse)) || courses[0] || '';
  const classes = catalogNames(allStudents
    .filter((student) => !courseName || sameCatalogName(student.courseName, courseName))
    .map((student) => student.className));
  const requestedClass = text(requestedFilters.className);
  const className = classes.find((name) => sameCatalogName(name, requestedClass)) || '';
  const requestedLessonId = text(requestedFilters.lessonId);

  const courseLessons = completedLessons
    .filter((lesson) => (!courseName || sameCatalogName(lesson.courseName, courseName))
      && lessonClassNames(lesson).some((name) => classes.some((existing) => sameCatalogName(name, existing)))
      && (!className || lessonClassNames(lesson).some((name) => sameCatalogName(name, className))))
  ...
  const students = allStudents
    .filter((student) => (!courseName || sameCatalogName(student.courseName, courseName))
      && (!className || sameCatalogName(student.className, className)))
  ...
  const attendsLesson = (student, lesson) => lessonClassNames(lesson).some((name) => sameCatalogName(name, student.className));
  const assignmentsFor = (studentId, scopedExercises = exercises) => scopedExercises
    .filter((exercise) => (!exercise.targetStudentId || exercise.targetStudentId === studentId)
      && students.some((student) => student.studentId === studentId && attendsLesson(student, lessons.find((lesson) => lesson.id === exercise.lessonId))));
  ...
  const studentRows = students.map((student) => {
    ...
    const studentLessons = lessons.filter((lesson) => attendsLesson(student, lesson));
    const present = studentLessons.filter((lesson) => attendanceKeys.has(`${student.studentId}\u241f${lesson.id}`)).length;
    ...
    return {
      ...
      attendanceExpected: studentLessons.length,
      attendanceRate: percentage(present, studentLessons.length),
  ...
  const trends = lessons.map((lesson) => {
    ...
    const correct = answered.filter((submission) => submission.correct).length;
    const lessonStudents = students.filter((student) => attendsLesson(student, lesson));
    const present = lessonStudents.filter((student) => attendanceKeys.has(`${student.studentId}\u241f${lesson.id}`)).length;
    return {
      lessonId: lesson.id,
      week: Number(lesson.teachingWeek) || 0,
      title: lesson.title || `第 ${lesson.teachingWeek || '-'} 周`,
      date: lesson.date || '',
      attendanceRate: percentage(present, lessonStudents.length),
      completionRate: percentage(answered.length, assignmentCount),
      accuracyRate: percentage(correct, answered.length),
    };
  });

  const attendancePresent = studentRows.reduce((sum, student) => sum + student.attendancePresent, 0);
  const attendanceExpected = studentRows.reduce((sum, student) => sum + student.attendanceExpected, 0);
